timetravel_dreamai extension의 사용법에 대한 Readme를 작성하려고해. Extension의 작동 순서대로 각각의 모듈에 대한 설명을 할거야. 모듈 설명은 하는 역할과 사용 방법 위주로 설명할거야. 아. timetravel_dreamai의 이름을 timetravel_summarization으로 바꿀거야. 현재 이름과 다르다는것을 염두해줘.
단계는 다음과 같아.
...
Note
- 이 문서에서 Timetravel 이란, 익스텐션 및 프레임워크를 지칭한다. 다만 공식적인(교수님 관점) Time Travel의 의미는 통합적인 시공간 분석 기능의 통칭이다. 즉 해당 프레임워크는 Time Travel을 도와주는 하나의 기능으로 볼 수 있음.
- 전체 프레임워크의 핵심이 되는 시공간 복원 기능을 제공하는 TimeTravel은 Extension, 이외에 분석을 도와주는 기능 (view_overlay, vlm_client, event_post_processing)은 Module로 정의한다. 즉, , 그 안에서 
- USD Composer의 기존 Extension인 Movie Capture Extension은 명칭 그대로 Extension을 유지함.
...
3. USD Composer의 Extension창에서 TimeTravel_dream_ai extension initialization.
3-1 Extension 기능
...
4. 복원된 장면에 objectID, Timestamp를 overlay. (View Overlay)
4-1 Module 기능
- Object ID overlay 선택
- Timestamp overlay 선택
4-2 구현
-view_overlay_core.py 는 overlay logic 담당
-view_overlay_window.py는 UI window 담당.
5. 동영상 추출 (Movie Capture Extension)
...
- 캡쳐 형태: mp4로 설정.
6. VLM Client를 통해 VLM Server에 동영상 전달
6-1 기능
- video: 앞서 생성한 동영상 이름
- Upload 버튼을 통해 업로드. Generate 버튼을 통해 결과 생성 요청 및 반환.
- Model: VLM 서버에서 실행중인 모델의 이름을 선택
- Preset: vlm_client_core.py에 미리 저장해둔 Prompt 선택
6-2 구현
- vlm_client_core.py
- vlm_cleint_window.py

7. Event Post Processing 모듈로 output을 Event list로 가공
7-1 기능 output json file 명